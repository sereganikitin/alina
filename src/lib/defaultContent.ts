...
export const DEFAULT_CONTENT = {
  hero: {
    ...
    // (0 — шапочка выпускника, 1 — рукопожатие, 2 — часы, 3 — человечки).
    // Сетка 2×2 заполняется по столбцам: 1–2 слева, 3–4 справа.
    stats: [
      { value: "8 лет", label: "в профессии" },
      { value: "10 лет", label: "частной практики" },
  ...
  principles: {
    // Схема-солнышко «с чем я работаю»: шесть разделов вокруг центра.
    // ⚠️ Порядок и количество разделов завязаны на геометрию кругов и
    //    лучей (SUN_GEOMETRY в Principles.tsx) — она подобрана под эти
    //    шесть блоков и под текущую длину текста. Меняя состав или сильно
    //    удлиняя пункты, геометрию нужно пересчитать, иначе текст вылезет
    //    за край круга.
    sun: [
      {
    ...
    // Вторая колонка под схемой — ограничения практики
    ...
    // Фото-арка убрана из «Принципов» (схема заняла всю ширину колонки);
    // поле оставлено на случай возврата.
  ...
  // Карусель: у каждого факта своя фотография (пролистывание синхронно).
  funFacts: {
