...
const iconCls = "h-7 w-7 sm:h-9 sm:w-9 2xl:h-10 2xl:w-10";
...
const clockCls = "h-8 w-8 sm:h-10 sm:w-10 2xl:h-11 2xl:w-11";
const ClockIcon = (
    ...
    >
      ...
      <div className="relative z-10 flex min-h-[100svh] flex-col">
        ...
        <div className="flex flex-1 items-center px-6 md:px-12">
          <ul className="grid max-w-[15rem] grid-cols-1 gap-y-4 sm:max-w-xs sm:gap-y-6 xl:max-w-lg xl:grid-cols-2 xl:gap-x-6 xl:gap-y-7 2xl:max-w-xl 2xl:gap-y-8">
            {/* три цифры из контента + постоянный бейдж «Конфиденциально» */}
            {c.hero.stats.slice(0, 3).map((s, i) => (
              <li key={s.label} className="flex items-center gap-2.5 sm:gap-3.5 2xl:gap-4">
                <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full border-[1.5px] border-terracotta text-terracotta sm:h-14 sm:w-14 2xl:h-16 2xl:w-16">
                  {STAT_ICONS[i]}
                </span>
                <span className="leading-tight">
                  <span className="block whitespace-nowrap font-display text-lg leading-none text-terracotta sm:text-2xl 2xl:text-[1.7rem]">
                    {s.value}
                  </span>
                  <span
            ...
            <li className="flex items-center gap-2.5 sm:gap-3.5 2xl:gap-4">
              <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full border-[1.5px] border-terracotta text-terracotta sm:h-14 sm:w-14 2xl:h-16 2xl:w-16">
                {LockIcon}
              </span>
              <span className="leading-tight">
                ...
                <span
                  className={`mt-1 block text-sm lowercase leading-tight tracking-normal transition-colors sm:text-base lg:whitespace-nowrap ${
                    v.dark ? "text-white/90 hover:text-white" : "text-foreground/75 hover:text-foreground"
                  }`}
                >
